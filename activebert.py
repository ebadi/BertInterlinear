...
for i in range(len(tokenized_sentence)):
    ...
    if i % 5 == 1:
        trans_options = [("brown", 0), (tokenized_sentence[i],
                                        0), ("dog", 0), ("lazy", 0)]
    else:
        trans_options = [(tokenized_sentence[i], 1)]

    sentecelist.append(
        (i, compound_word, (toklist, toklist_id), trans_options))
    print(sentecelist[i])
...
for i in range(len(sentecelist)):
    ...
    if len(trans_options) == 1:
        ...
        masked_sentencelist.append(
            (indx, "[MASK]", ([mask_token], [mask_token_id]), trans_options))
...
for j in [7,6,5,4,3,2,1]:
  ...
  for i in range(len(masked_sentencelist)):
    ...
    sorted_sentecelist.append((indx, word, (toklist, toklist_id), ranked_pairs))
